...
Model_2=glm(Loan_Status_Der~Credit_History+ApplicantIncome+LoanAmount+CoapplicantIncome+Property_Area,
            data=Credit_Risk_Train_data_filled,family = binomial)
summary(Model_2)
step(Model_1)
...
Model_3=glm(Loan_Status_Der~Property_Area+Credit_History+LoanAmount,data=Credit_Risk_Train_data_filled,family = binomial)
summary(Model_3)
library("MASS")

Model_4=glm(Loan_Status_Der~Property_Area+Credit_History+LoanAmount+ApplicantIncome+CoapplicantIncome,data=Credit_Risk_Train_data_filled,family = binomial)

Credit_Risk_Test_data_filled$pred_test=predict(Model_4,newdata=Credit_Risk_Test_data_filled,type="response")
...
rf=randomForest(Loan_Status_Der ~ Married + Education + LoanAmount + 
                  Credit_History + Property_Area,
                data=Credit_Risk_Train_data_filled,ntree=100,mtry=5)

rf=randomForest(Loan_Status_Der ~ .-Loan_Status-Loan_ID,
                data=Credit_Risk_Train_data_filled,ntree=100,mtry=5)
summary(rf)
rf$importance
library(caret)
...
Model3=glm(Loan_Status_Der~LoanAmount+Property_Area+Credit_History,data=Credit_Risk_Train_data_filled,family = binomial)
summary(Model3)
